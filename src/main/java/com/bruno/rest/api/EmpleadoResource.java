...
public class EmpleadoResource {
	...
	public Response findEmpleadoById(@PathParam("id") Long id)
	...
	@Operation(summary = "Buscar empleado por critérios", description = "Permite buscar empleado aplicando múltiplos critérios como nombre, estado, fechas, entre outros.", responses = {
			@ApiResponse(responseCode = "200", description = "Lista de empleados encontradas", content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = Results.class))),
	...
	@POST
	@Operation(summary = "Crea un nuevo empleado", description = "Permite crear un empleado enviando datos a través de Query Params.", responses = {
			@ApiResponse(responseCode = "201", description = "Empleado creado exitosamente"),
			@ApiResponse(responseCode = "400", description = "Error en la creación del empleado") })
	@Produces(MediaType.APPLICATION_JSON)
	public Response crearEmpleado(@QueryParam("nombre") String nombre, @QueryParam("apellido") String apellido,
			@QueryParam("email") String email, @QueryParam("fechaAlta") String fechaAltaStr,
			@QueryParam("rolId") Integer rolId) {
		try {
	...
	@DELETE
	@Operation(summary = "Eliminar un empleado", description = "Elimina un empleado a partir de su ID pasado como Query Param.", responses = {
	...
	public Response deleteEmpleado(@QueryParam("id") Long id) {
		try {
			if (id == null) {
				return Response.status(Status.BAD_REQUEST).entity("Debe proporcionar un ID válido.").build();
			}
			...
			return Response.status(Status.INTERNAL_SERVER_ERROR)
					.entity("Error al eliminar el empleado: " + e.getMessage()).build();
		}
	}

	@PUT
	@Operation(summary = "Atualizar dados de um empregado", description = "Atualiza um empregado existente com base no ID fornecido.", responses = {
			...
			@ApiResponse(responseCode = "500", description = "Erro interno do servidor") })
	@Produces(MediaType.APPLICATION_JSON)
	public Response updateEmpleado(@QueryParam("id") Long id, @QueryParam("nombre") String nombre,
			@QueryParam("apellido") String apellido, @QueryParam("email") String email,
			@QueryParam("fechaAlta") String fechaAltaStr, @QueryParam("rolId") Integer rolId,
	...
	@Produces(MediaType.APPLICATION_JSON)
	@Operation(
	    summary = "Autenticar empregado",
	    description = "Autentica um empregado com ID e senha fornecidos no corpo da requisição.",
	    responses = {
	        @ApiResponse(responseCode = "200", description = "Autenticação bem-sucedida", content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = EmpleadoDTO.class))),
	        @ApiResponse(responseCode = "401", description = "Credenciais inválidas"),
	        @ApiResponse(responseCode = "400", description = "Erro na autenticação"),
	        @ApiResponse(responseCode = "500", description = "Erro interno do servidor")
	    }
	)
	public Response autenticarEmpleado(EmpleadoDTO credenciais) {
	    try {
	        if (credenciais.getId() == null || credenciais.getContrasena() == null || credenciais.getContrasena().isEmpty()) {
	            return Response.status(Status.BAD_REQUEST).entity("ID e senha são obrigatórios").build();
	        }
	        
	        EmpleadoDTO empleado = empleadoService.autenticar(credenciais.getId(), credenciais.getContrasena());
	        if (empleado != null) {
	        ...
	        e.printStackTrace();
	        return Response.status(Status.INTERNAL_SERVER_ERROR).entity("Erro ao autenticar o empregado").build();
	    }
	}
}
